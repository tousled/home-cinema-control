...
class PollingPlaybackObservationStrategyTest(unittest.TestCase):
    ...
    def test_does_not_stop_at_old_ten_second_window(self):
        oppo = RecordingOppoPlayback(
            states=[
                _state(PlayerPlaybackStatus.PLAY, PlayerPlaybackLifecyclePhase.ACTIVE),
                _state(PlayerPlaybackStatus.PLAY, PlayerPlaybackLifecyclePhase.ACTIVE),
            ...
            positions=[
                PlayerPlaybackPosition(current_seconds=3553, total_seconds=3563),
            ],
        )
        orchestrator = PollingPlaybackObservationStrategy(
            media_player=oppo,
    ...
    def test_stops_when_polling_detects_next_file_after_feature_end(self):
        ...
        oppo = RecordingOppoPlayback(
            ...
            positions=[
                PlayerPlaybackPosition(current_seconds=7753, total_seconds=7756),
        ...
        self.assertEqual(7753, result.position_seconds)
        ...
        self.assertEqual(2, oppo.position_calls)
        self.assertEqual([7753], [call["position_seconds"] for call in progress.calls])
