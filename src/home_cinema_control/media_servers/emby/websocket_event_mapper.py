...
def emby_sessions_start_message() -> str:
    return '{"MessageType":"SessionsStart", "Data": "0,1500"}'


class EmbyWebsocketEventMapper:
    def __init__(self, *, emby_session) -> None:
        self._session = emby_session
    ...
    def map(self, raw_message: str) -> MediaServerWebsocketEvent:
        ...
        if message_type == "Playstate":
            return MediaServerWebsocketEvent(
                ...
                command=command_from_emby_playstate_message(data or {}),
            )
        ...
        if message_type == "GeneralCommand":
            return MediaServerWebsocketEvent(
                kind=_EventKind.PLAYBACK_COMMAND,
                raw_type=message_type,
                command=command_from_emby_general_command_message(data or {}),
            )
...
def command_from_emby_playstate_message(data: dict[str, Any]) -> MediaServerCommand:
...
def command_from_emby_general_command_message(
    data: dict[str, Any],
) -> MediaServerCommand:
    name = data.get("Name")
